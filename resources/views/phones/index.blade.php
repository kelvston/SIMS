...
<div class="container mx-auto bg-white p-8 rounded-lg shadow-md mt-10">
    ...
    @else
        <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach ($phones as $phone)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->imei }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->brand->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->model }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->color }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->storage_capacity }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ number_format($phone->selling_price, 2) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $phone->received_at->format('Y-m-d H:i') }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
        ...
        <div class="mt-6">
            {{ $phones->links('pagination::tailwind') }} <!-- Laravel's default Tailwind pagination view -->
        </div>
    @endif
@endsection
